...
export function BackupSettings() {
  ...
  const restoreMutation = useMutation({
    mutationFn: (filename: string) =>
      axios.post("/api/backup/restore", { filename }),
    onSuccess: () => {
      toast.success("Database restored successfully. The page will reload.");
      setRestoreTarget(null);
      // Give the server a moment to re-initialize, then reload so the UI
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Automatically snapshot the SQLite database on a schedule. Backups
            are verified for integrity and old copies are pruned automatically.
          </CardDescription>
        </CardHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Absolute or relative path where backup files are
                          stored. The directory will be created if it does not
                          exist.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
        ...
        <AlertDialogContent>
          ...
          <div className="space-y-3 text-sm">
            ...
            <p className="text-muted-foreground">
              ...
              {restoreTarget &&
                format(
                  new Date(restoreTarget.createdAt),
                  "MMMM d, yyyy 'at' HH:mm"
                )}{" "}
              &mdash; {restoreTarget && formatBytes(restoreTarget.sizeBytes)}
            </p>
            ...
            <div className="rounded-md border border-destructive/40 bg-destructive/5 p-3 space-y-1.5">
              ...
              <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
                <li>All connections, jobs, job runs, and transfer logs will be replaced with the backup contents.</li>
                <li>A safety snapshot of the current database is automatically created before the restore begins.</li>
                <li>The page will reload automatically after a successful restore.</li>
              </ul>
            </div>
          </div>
          ...
          <AlertDialogFooter>
            ...
            <Button
              ...
              onClick={() =>
                restoreTarget && restoreMutation.mutate(restoreTarget.filename)
              }
            >
